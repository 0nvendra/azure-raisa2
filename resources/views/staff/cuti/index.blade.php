            ...
            <div class="box-body">
                ...
                <div style="margin-bottom: 20px">
                    @if ($sisacuti->sisa_cuti == 0)
                        <button type="button" class="btn btn-danger"><i class="fa Lfa-info-circle" aria-hidden="true"></i>
                            Remaining Days Off : {{ $sisacuti->sisa_cuti }} days</button>
                    @else
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target=".tambahspd"><span
                                class="glyphicon glyphicon-plus"></span> Add Request</button>
                        <button type="button" class="btn btn-danger"><i class="fa Lfa-info-circle" aria-hidden="true"></i>
                            Remaining Days Off : {{ $sisacuti->sisa_cuti }} days</button>
                    @endif
                </div>
                            ...
                            @foreach ($cuti_karyawan as $k => $d)
                                <tr>
                                    ...
                                    <td>
                                        <center>
                                            @if ($d->status == 0)
                                                <button class='btn btn-xs btn-default delete' data-id="{{ $d->id }}"
                                                    style="color: dodgerblue;"><span
                                                        class='glyphicon glyphicon-trash'></span></button>
                                            @elseif($d->status == 1)
                                                <a href="{{ url('downloadcuti', $d->id) }}" class="btn btn-default btn-xs"
                                                    style="color: dodgerblue;"><span
                                    ...
                                    <td>
                                        <center>
                                            ...
                                            @if ($d->status == 0)
                                                <span></span>
                                            @elseif($d->status == 1)
                                                <a href="{{ route('showUpload', $d->id) }}" class="btn btn-default btn-xs"
                                                    style="color: dodgerblue;">Upload file</span></a>
                                                @if ($d->file_scan != null || $d->file_scan != '')
                                                    <a href="{{ url('uploads/Cuti/' . $d->file_scan) }}" target="_blank"
                                                        class="btn btn-default btn-xs" style="color: dodgerblue;">View
                                                        file</a>
                                                @else
                                                @endif
                                            @else
