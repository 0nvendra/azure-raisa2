...
Route::get('payroll-login', function () {
	return view('payroll/login');
});
Route::post('payroll-login', 'HomeController@index');

// quick fix query by noven 9/24/22
// Route::get('quick-fix', function () {
// 	// return view('login-form');
// 	$karyawans = Employee::where('nik', 'like', '%AI%')->get();
// 	foreach ($karyawans as $key => $value) {
// 		# code...
// 		$ts = TimeSheetUser::where('id_karyawan', $value->nik)->update([
// 			'id_user' => $value->id,
// 		]);
// 		// $ts->Save();
// 	}
...
Route::group(['middleware' => 'auth'], function () {
	...
	Route::group(['namespace' => 'HRD\Psikotes\CFIT', 'middleware' => 'auth.hr'], function () {
		// CFIT 
		// Master Data Kategori
		Route::get('kategori-cfit', 'KategoriController@index');
		Route::get('kategori-cfit/create', 'KategoriController@create');
		Route::post('kategori-cfit/store', 'KategoriController@store');
		Route::get('kategori-cfit/{id}', 'KategoriController@edit')->name('editkategoricfit');
		Route::post('kategori-cfit/update/{id}', 'KategoriController@update')->name('updatekategoricfit');
		Route::post('kategori-cfit/{id}', 'KategoriController@destroy')->name('deletekategoricfit');

		// Master Data Statement
		Route::get('statement-cfit', 'StatementController@index');
		Route::get('statement-cfit/create', 'StatementController@create');
		Route::post('statement-cfit/store', 'StatementController@store');
		Route::get('statement-cfit/{id}', 'StatementController@edit')->name('editstatementcfit');
		Route::post('statement-cfit/update/{id}', 'StatementController@update')->name('updatestatementcfit');
		Route::post('statement-cfit/{id}', 'StatementController@destroy')->name('deletestatementcfit');

		// Report CFIT
		Route::get('report-cfit', 'ReportController@index');
		Route::get('report-cfit/scoring/{id}', 'ReportController@show')->name('show-report-cfit');
	});
